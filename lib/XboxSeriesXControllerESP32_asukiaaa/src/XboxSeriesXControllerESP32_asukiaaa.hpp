 ...
 private:
  ...
  bool connectToServer(const NimBLEAdvertisedDevice* advDevice) {
    ...
    if (!pClient) {
      ...
      // NimBLE 2.x: 2e parametre = asyncConnect (false = sync, attend la connexion)
    ...
    while (!pClient->isConnected()) {
      ...
      // NimBLE 2.x: reconnexion sync par adresse
      pClient->connect(pClient->getPeerAddress(), false);
      --retryCount;
    }
#ifdef XBOX_SERIES_X_CONTROLLER_DEBUG_SERIAL
